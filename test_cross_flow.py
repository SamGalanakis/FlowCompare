...
load_path = 'save/conditional_flow_compare/clear-field-2387_e560_model_dict.pt' # 'save/conditional_flow_compare/clear-field-2387_e400_model_dict.pt'#'save/conditional_flow_compare/drawn-water-2240_e0_b31999_model_dict.pt'"
...
def dataset_view(dataset,index,multiple =3.,gen_std=0.6,show=False):
    with torch.no_grad():
        ...
        vars = ['gen_given_0','gen_given_1','change_1_given_0','change_0_given_1','voxel_0','voxel_1']
        ...
        for key,val in return_dict['voxels'].items():
            ...
            context_0,voxel_1,context_1,voxel_0,inverse_voxel_0,inverse_voxel_1,z_voxel_center = [to_dev(x,device) for x in val]
            print(f'Vox center {z_voxel_center}')
            #height_adjust = torch.tensor([0.,0.,z_voxel_center.item(),0.,0.,0.]).to(device)
            
            
            log_prob_1_given_0 = calc_change(context_0, voxel_1,model_dict,config).cpu()
            log_prob_0_given_0 = calc_change(context_0, voxel_0,model_dict,config).cpu()
            log_prob_0_given_1 = calc_change(context_1, voxel_0,model_dict,config).cpu()
            log_prob_1_given_1 = calc_change(context_1, voxel_1,model_dict,config).cpu()
            

            gen_given_0 = make_sample(1024,context_0.unsqueeze(0),model_dict,config,sample_distrib=sample_distrib)
            gen_given_1 = make_sample(1024,context_1.unsqueeze(0),model_dict,config,sample_distrib=sample_distrib)
            ...
            procesed_dict['change_1_given_0'].append(log_prob_to_color(log_prob_1_given_0,log_prob_0_given_0,multiple = multiple).cpu())
            procesed_dict['change_0_given_1'].append(log_prob_to_color(log_prob_0_given_1,log_prob_1_given_1,multiple = multiple).cpu())
        ...
        procesed_dict = {key:torch.cat(val,dim=0) for key,val in procesed_dict.items()}
        ...
        fig_0_given_1 = view_cloud_plotly(procesed_dict['voxel_0'][:,:3],procesed_dict['change_0_given_1'],colorscale='Bluered',show_scale=True,show=show,title='Extract 0 given 1')
        fig_1_given_0 = view_cloud_plotly(procesed_dict['voxel_1'][:,:3],procesed_dict['change_1_given_0'],colorscale='Bluered',show_scale=True,show=show,title='Extract 1 given 0')
