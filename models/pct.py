...
class NeighborhoodEmbedder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn2(self.conv2(x)))
        x = x.permute(0, 2, 1)
        new_xyz, new_feature = sample_and_group(npoint=n_points, radius=0.15, nsample=32, xyz=xyz, points=x)         
        feature_0 = self.gather_local_0(new_feature)
        feature = feature_0.permute(0, 2, 1)
...
if __name__ == '__main__':
    ...
    argsz = args(0.5)
    model = NeighborhoodEmbedder(6).cuda()
    points  = torch.randn((5,2000,6)).cuda()
    result = model(points)
    pass