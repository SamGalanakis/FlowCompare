...
def visualize_change(fig_getter, index_range):
    ...
    index_selector_options = [{'label': key, 'value': key}
                              for key in index_range]
    global multiple
    global index
    multiple = 3.
    ...
    def index_chooser(multiple, gen_std, index):
        ...
        print(multiple)
        graph_0, graph_1, graph_0_given_1, graph_1_given_0, gen_given_1, gen_given_0 = fig_getter(
            index, float(multiple), float(gen_std))
